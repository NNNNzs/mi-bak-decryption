...
async function extractTarWithProgress(tarFilePath, outputDir) {
  return new Promise((resolve, reject) => {
    try {
      ...
      console.log(`开始解压TAR文件到目录: ${outputDir}`);
      
      // 处理每个文件条目
      extract.on('entry', (header, stream, next) => {
        // 处理文件路径中的特殊字符，但保留路径分隔符
        // 将路径分割为目录和文件名部分
        ...
        // 重新组合路径，保留原始目录结构
        const normalizedPath = pathParts.length > 0 
          ? path.join(...pathParts, normalizedFileName)
          : normalizedFileName;
        ...
        if (header.type === 'directory') {
          ...
          dirCount++;
          console.log(`创建目录 (${dirCount}): ${filePath}`);
          stream.resume();
          next();
      ...
      extract.on('finish', () => {
        progressBar.stop();
        console.log(`解压完成: 创建了 ${dirCount} 个目录和 ${fileCount} 个文件`);
        resolve();
      });
